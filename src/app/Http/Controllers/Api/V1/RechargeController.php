        ...
        try {
            ...
            switch ($rechargeChannel->slug) {
                case PlatformType::Yudrsu:
                    //abort_if(Carbon::now()->between(Carbon::make("22:50"), Carbon::make("23:10")), 400, Lang('充值维护'));
            ...
            switch ($rechargeChannel->slug) {
                ...
                case PlatformType::USDT1788:
                    $pay_url = Usdt1788Service::make()->payIn($user, $order, $rechargeChannelList, $redirect_url);
                    if ($pay_url) {
                        $res['pay_url'] = $pay_url;
                        $res['order_sn'] = $order->order_sn;
                        return $this->response($res);
