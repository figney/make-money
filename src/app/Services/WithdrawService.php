...
class WithdrawService extends BaseService
{
    ...
    public function deductAwardHandle(UserWithdrawOrder $userWithdrawOrder)
    {
        ...
        //扣除自己的赠送金
        $this->deductAwardHandleFn($user, $amount, $userWithdrawOrder, WalletLogType::WithdrawDeductRechargeAward);
        ...
        for ($i = 1; $i <= 10; $i++) {
            ...
            try {
                if ($invite_id > 0) {
                    ...
                    $deduct_money = $this->deductAwardHandleFn($invite_user, $amount, $userWithdrawOrder, WalletLogType::WithdrawFriendDeductRechargeAward);
                    //上级有被扣除赠送金
                    if ($deduct_money > 0) {
                        //上级统计数据下级产生赠送金余额扣除
    ...
    private function deductAwardHandleFn(User $user, $amount, $userWithdrawOrder, $action_type)
    {
        $walletService = new WalletService();

        list($deduct_money, $deduct_items, $beyond_money) = $this->deductAward($user, $amount);
     ...
     * @return array
     */
    public function deductAward($user, $money)
    {
        $list = UserAwardRecord::query()->where('user_id', $user->id)->where('is_deduct', true)->orderByDesc('rate')->where('residue_amount', '>', 0)->get();
        //需要扣除的金额
        $deduct_money = 0;
        //需要扣除的记录
